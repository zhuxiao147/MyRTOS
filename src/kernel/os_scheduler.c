...
void os_scheduler(void) {
    ...
    for (os_int32_t i = 0; i < OS_MAX_TASKS; i++) {
        if (os_task_list[i].state == OS_TASK_READY) {
            if (os_task_list[i].scheduler_count >= scheduler_count &&
                os_task_list[i].priority >= priority) {
                // Found a ready task with higher priority or equal priority and lower
                scheduler_count = os_task_list[i].scheduler_count;
                priority = os_task_list[i].priority;
                nextTask = &os_task_list[i];
            }
            os_task_list[i].scheduler_count++;
        }
    }
    ...
    os_currentTask->scheduler_count = 0;
    nextTask->state = OS_TASK_RUNNING;
    nextTask->scheduler_count = 0;
